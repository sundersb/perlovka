...
query(void)
{
    ...
    static GimpParamDef args[] =
        {
            ...
            {GIMP_PDB_INT32, "grid", "Grain detection grid to use: odd, even or both"},
            {GIMP_PDB_INT32, "matching", "Fields matching: soft or strict"},
            {GIMP_PDB_INT32, "resolver", "Compensation mode: minimal, least-of-max, largest-of-min, maximal"},
        };
    ...
    gimp_install_procedure(
        ...
        "Alexander Belkov <sunders@github.com>",
        "Alexander Belkov",
        "2025",
        N_("Per_lovka Degranulation..."),
        "RGB*, GRAY*",
        GIMP_PLUGIN,
        G_N_ELEMENTS(args), 0,
        args, NULL);
...
void fix_options()
{
    ...
    if (settings.resolver < RESOLVER_MINIMAL || settings.resolver > RESOLVER_MAXIMAL)
...
load_params(gint nparams, const GimpParam *param)
{
    ...
    case GIMP_RUN_NONINTERACTIVE:
        ...
        settings.iterations_limit = param[PERLOVKA_PARAM_ITERATIONS].data.d_int32;
...
#ifdef PERLOVKA_USE_GEGL
    format = data->color_count == 1
                 ? babl_format("Y' u8")
                 : babl_format("R'G'B' u8");

    gegl_buffer_get(buffer,
                    GEGL_RECTANGLE(0, 0, data->width, data->height),
                    1.0,
                    format, buf,
                    channels * data->width, GEGL_ABYSS_CLAMP);
#else
 ...
 */
void clean_data(struct PerlovkaData *data)
{
    int index;
 ...
 */
void normalize(int *data, size_t size)
{
    int amplitude;
    double q;
 ...
 */
GimpPDBStatusType denoize(struct PerlovkaData *data)
{
    ...
    if (conditions.show_progress)
    {
        ...
        conditions.progress_tick = 1.0 / settings.iterations_limit / data->color_count;
