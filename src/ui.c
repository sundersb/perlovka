...
gboolean
show_perlovka_dialog (PerlovkaPluginSettings *settings)
{
  GtkWidget *dlg;
  GtkWidget *main_vbox;
  GtkWidget *frame;
  ...
  adj = (GtkAdjustment *)gtk_adjustment_new (settings->radius, 1, 50, 1, 10,
                                             0);
  ...
  adj = (GtkAdjustment *)gtk_adjustment_new (settings->iterations_limit, 1,
                                             100, 1, 10, 0);
  spin = gimp_spin_button_new (adj, 1.0, 0);
  gtk_spin_button_set_numeric (GTK_SPIN_BUTTON (spin), TRUE);
